# @summary Kubernetes installation
#
...
class kubeinstall (
  ...
  Stdlib::Unixpath $crio_config_path,
  ...
  Optional[Kubeinstall::Address] $control_plane_endpoint,
  Optional[Integer] $docker_mtu,
  Optional[String] $network_bridge_ip,
  Optional[Integer] $calico_mtu,
  Optional[Boolean] $cri_selinux,
  String $cluster_name,
  ...
  Stdlib::Unixpath $cri_socket         = $container_runtime ? {
    'docker' => $kubeinstall::params::docker_socket,
    default => $kubeinstall::params::crio_socket,
  },
  Stdlib::Fqdn $service_dns_domain     = $kubeinstall::params::service_dns_domain,
  Stdlib::IP::Address $service_cidr    = $kubeinstall::params::service_cidr,
  Stdlib::IP::Address $pod_network_cidr = $kubeinstall::params::pod_network_cidr,
